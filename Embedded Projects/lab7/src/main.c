...
GPIOA->MODER &= ~(3<<16);
GPIOA->MODER |= (2<<16); // pa8 for ch1
GPIOA->MODER &= ~(3<<18);
GPIOA->MODER |= (2<<18); // pa9 for  ch2
GPIOA->MODER &= ~(3<<20);
GPIOA->MODER |= (2<<20); // pa10 for  ch3
GPIOA->MODER &= ~(3<<22);
GPIOA->MODER |= (2<<22); // pa11 for  ch2

//GPIOA->AFR[1] &= ~0xf; // setting the alternate function
GPIOA->AFR[1] |= 0x2;
GPIOA->AFR[1] |= (0x2) << 4;
GPIOA->AFR[1] |= (0x2) << 8;
GPIOA->AFR[1] |= (0x2) << 12;
...
RCC->APB2ENR |= RCC_APB2ENR_TIM1EN; //  clock to tim1e

TIM1->BDTR  &= ~(1<<15);
TIM1->BDTR |= (1<<15);

TIM1->PSC = 0 ; // psc value
TIM1->ARR = 2400 - 1; // arr value
...
{
    TIM6->SR &= ~TIM_SR_UIF ;
    //DAC->SWTRIGR |= DAC_SWTRIGR_SWTRIG1; // triger the dac
    offseta += stepa;
    offsetb += stepb;
    offsetc += stepc;
    offsetd += stepd;
    ...
    if ((offsetb >> 16) >= N) {// If we go past the end of the array,
    offsetb -= N <<16; // wrap around with same offset as overshoot.
    }
    if ((offsetc >> 16) >= N) {// If we go past the end of the array,
    offsetc -= N <<16; // wrap around with same offset as overshoot.
    }
    if ((offsetd >> 16) >= N) {// If we go past the end of the array,
    offsetd -= N<<16; // wrap around with same offset as overshoot.
    }
...
void setrgb(int rgb)
{
    ...
    double blue = (rgb % 16)  + (((rgb / 16) % 16) *  10);

    double green = ((rgb /256) % 16)  + (((rgb / (256*16) % 16) *  10));
    double red = ((rgb /(256 * 256)) % 16)  + (((rgb / (256*256*16) % 16) *  10));



    int b = ((1 - (blue / 100)) * (TIM1->ARR + 1));
                if (b %2 != 0)
                {
                    b++;
                }
